...
Future<List<AEClinicalDiscipline>> HrAddEmplyClinicalDisciplinApi(BuildContext context, int deptID) async {
...
Future<List<AEClinicalCity>> HrAddEmplyClinicalCityApi(BuildContext context,
    // int deptID
    ) async {
  List<AEClinicalCity> itemsList = [];
  try {
    final response = await Api(context)
        .get(path: HRModuleAEClinicalRepository.getAddEmployeeTypeCity(
        // deptID
    ));
    if (response.statusCode == 200 || response.statusCode == 201) {
      for (var item in response.data) {
...
Future<List<AEClinicalReportingOffice>> HrAddEmplyClinicalReportingOfficeApi(BuildContext context, int companyID) async {
  List<AEClinicalReportingOffice> itemsList = [];
  try {
    final response = await Api(context)
        .get(path: HRModuleAEClinicalRepository.getAddEmplyReportingOffice(companyID));
    if (response.statusCode == 200 || response.statusCode == 201) {
      for (var item in response.data) {
        itemsList.add(
...
///zone drop down
Future<List<AEClinicalZone>> HrAddEmplyClinicalZoneApi(BuildContext context,
    // int companyID,
    // int officeID,
    // int countyID,
    // int pageNo,
  ...
  try {
    final response = await Api(context)
        .get(path: HRModuleAEClinicalRepository.getAddEmplyZone(
