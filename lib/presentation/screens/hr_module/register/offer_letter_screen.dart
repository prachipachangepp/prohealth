...
class _OfferLetterScreenState extends State<OfferLetterScreen> {
  ...
  List<GlobalKey<_DynamicConatinerState>> containers = [];

  void _addContainer() {
    setState(() {
      containers.add(GlobalKey<_DynamicConatinerState>() );
    });
  }
                          ...
                          onConfirm: () async {
                            ...
                            try {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                      content: Text('Enrollment data save')));
                            } catch (e) {
...
class DynamicConatiner extends StatefulWidget {
  const DynamicConatiner({super.key});

  @override
  _DynamicConatinerState createState() => _DynamicConatinerState();
}

class _DynamicConatinerState extends State<DynamicConatiner> {
                        ...
                        if (snapshot.connectionState ==
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color:
                                  const Color(0xff686464).withOpacity(0.5),
                                  width: 1), // Black border
                              borderRadius:
                              BorderRadius.circular(6), // Rounded corners
                            ),
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: TabBarView(
                          ...
                          children: [
                            FutureBuilder<List<ZipcodeByCountyIdData>>(
                              future: getZipcodeByCountyId(
                                  context: context, countyId: selectedCountyId!),
                              builder: (BuildContext context, snapshot) {
                                if (snapshot.connectionState ==
                                    ConnectionState.waiting) {
                                  return SizedBox();
